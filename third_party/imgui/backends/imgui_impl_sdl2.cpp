...
bool ImGui_ImplSDL2_ProcessEvent(const SDL_Event* event) {
    ...
    switch (event->type) {
        ...
        case SDL_TEXTINPUT: {
            // printf("[SDL_TEXTINPUT]: %s, editorLength: %d, editorLength: %d\n",
            //     event->text.text, editorLength, editorLength);
            io.AddInputCharactersUTF8(event->text.text);
            return true;
        }
        case SDL_TEXTEDITING: {
            // printf("[SDL_TEXTEDITING]: %s (cursor position within text: %d, editorLength: %d)\n",
            //        event->edit.text,
            //        event->edit.start,
            //        editorLength);
            // if (event->edit.start > editorLength ) {
            //     io.AddInputCharacter(event->edit.text[event->edit.start-1]);
            // }
            editorLength = event->edit.start;
            return true;
        }
        case SDL_KEYDOWN:
        case SDL_KEYUP: {
            // printf("[SDL_KEYUP]: %d editorLength: %d\n",
            //     ImGui_ImplSDL2_KeycodeToImGuiKey(event->key.keysym.sym), editorLength);
            ImGui_ImplSDL2_UpdateKeyModifiers((SDL_Keymod)event->key.keysym.mod);
            ImGuiKey key = ImGui_ImplSDL2_KeycodeToImGuiKey(event->key.keysym.sym);
            if (editorLength != 0 && (key ==ImGuiKey_Delete || key==ImGuiKey_Backspace)) {
                return true;
            }
        ...
        case SDL_WINDOWEVENT: {
            ...
            if (window_event == SDL_WINDOWEVENT_CLOSE || window_event == SDL_WINDOWEVENT_MOVED || window_event ==
...
void ImGui_ImplSDL2_NewFrame() {
    ...
    if (bd->PendingMouseLeaveFrame && bd->PendingMouseLeaveFrame >= ImGui::GetFrameCount() && bd->MouseButtonsDown ==
        0) {
        bd->MouseWindowID = 0;
        bd->PendingMouseLeaveFrame = 0;
